...
Guild::~Guild() {
    ...
    cout << "Liberando " << personajes.size() << " personajes..." << endl;

    // Personajes es la colección que se va a recorrer.
    // En cada iteración, par representa un elemento de esa colección.
    for (auto& par: personajes)// Es un for basado en rango.
        // auto: Para que el compilador reconozca el tipo.
        // y & para trabajar directamente sobre el elemento.
        {
    ...
    personajes.clear(); //limpio el mapa.
...
void Guild:: agregarPersonaje(Personaje* personaje) {
    ...
    if (personajes.find(nombre) != personajes.end())
...
Personaje * Guild::buscarPersonaje(string nombre) {
    ...
    auto it = personajes.find(nombre); // El auto le pide al compilador que deduzca el tipo de it.
...
int Guild:: getCantidadPersonajes() {
    ...
    return personajes.size(); // Tambine retorna el numero total, no solo los vivos.
}